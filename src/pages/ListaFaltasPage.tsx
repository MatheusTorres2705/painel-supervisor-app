...
function parseDateTs(d: string): number {
  if (!d) return 0;
  const s = d.trim();
...
type FarolStatus = "vermelho" | "amarelo" | "verde";

function getFarolRow(r: { nropedcompra: number | null; dtentregav: string; dtinicioprev: string }): FarolStatus {
...
const SEM_CHASSI = "Sem chassi";
const linhaDe = (r: FaltaDetalheRow) => r.linha || SEM_LINHA;
const chassiDe = (r: FaltaDetalheRow) => r.chassi || SEM_CHASSI;

type Contagem = { itens: number; produtos: number; vermelho: number; amarelo: number; verde: number };
type BarcoGrupo = Contagem & { chave: string; linha: string; chassi: string };
type LinhaGrupo = Contagem & { linha: string; barcos: BarcoGrupo[] };
 ...
 */
function contar(rows: FaltaDetalheRow[]): Contagem {
...
const porGravidade = (a: Contagem, b: Contagem) =>
  b.vermelho - a.vermelho || b.amarelo - a.amarelo || b.itens - a.itens;

function agruparPorLinhaEBarco(rows: FaltaDetalheRow[]): LinhaGrupo[] {
